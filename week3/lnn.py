...
def LNN_2_entropy(x, k=5, tr=30, bw=0):
    ...
    for i in range(N):
        ...
        for neighbor in list_knn:
            ...
            S2 += (dis.transpose() * dis / (bw[i] ** 2)) * exp(
                -dis * dis.transpose() / (2 * bw[i] ** 2))
        ...
        if (det_Sigma < (1e-4) ** d):
            ...
            local_est[i] = -log(S0) + log(N - 1) + 0.5 * d * log(
                2 * pi) + d * log(bw[i]) + 0.5 * log(det_Sigma) + 0.5 * \
                                                                  offset[0][0]
...
def _3LNN_2_mi(data, split, k=5, tr=30):
    ...
    N = len(data)
    return LNN_2_entropy(x, k, tr) + LNN_2_entropy(y, k, tr) - LNN_2_entropy(
        data, k, tr)
...
def LNN_1_entropy(x, k=5, tr=30, bw=0):
    ...
    for i in range(N):
        ...
        local_est[i] = -log(S0) + log(N) + 0.5 * d * log(2 * pi) + d * log(
            bw[i]) + 0.5 * offset[0][0]
...
def _KSG_mi(data, split, k=5):
    ...
    for i in range(N):
        ans += -log(
            len(tree_y.query_ball_point(y[i], knn_dis[i], p=2)) - 1) / N - log(
...
def _3LNN_1_mi(data, split, k=5, tr=30):
    ...
    x = data[:, :split]
    y = data[:, split:]
    return LNN_1_entropy(x, k, tr) + LNN_1_entropy(y, k, tr) - LNN_1_entropy(
        data, k, tr)
...
def _3LNN_1_KSG_mi(data, split, k=5, tr=30):
    ...
    tree_xy = ss.cKDTree(data)
    knn_dis = [tree_xy.query(point, k + 1, p=2)[0][k] for point in data]
    return LNN_1_entropy(x, k, tr, bw=knn_dis) + LNN_1_entropy(y, k, tr,
                                                               bw=knn_dis) - LNN_1_entropy(
        data, k, tr, bw=knn_dis)
...
def _3LNN_2_KSG_mi(data, split, k=5, tr=30):
    ...
    tree_xy = ss.cKDTree(data)
    knn_dis = [tree_xy.query(point, k + 1, p=2)[0][k] for point in data]
    return LNN_2_entropy(x, k, tr, bw=knn_dis) + LNN_2_entropy(y, k, tr,
                                                               bw=knn_dis) - LNN_2_entropy(
        data, k, tr, bw=knn_dis)
